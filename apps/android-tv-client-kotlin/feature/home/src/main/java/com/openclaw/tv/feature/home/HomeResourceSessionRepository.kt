...
internal open class HomeResourceSessionRepository(
    ...
    open suspend fun load(sessionToken: String): ResolvedResourceSession? {
        return try {
            ...
            val stored = session.toStoredResourceSession(nowEpochMs())
                .toClientVisibleResourceSession(nowEpochMs())
            cacheStore?.save(stored)
            ...
            cached?.toResolvedResourceSession(source = ResourceSessionSource.CACHE)
        }
    }
