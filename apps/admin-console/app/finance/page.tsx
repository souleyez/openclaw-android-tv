...
import { fetchAdminFinanceSnapshot, formatDateTime, formatGenericStatus, formatNumber } from "@/lib/admin-api";
...
export default async function FinancePage({ searchParams }: FinancePageProps) {
  ...
  const filteredOrders = finance.recentOrders.filter((item) => {
    ...
    const haystack = [item.id, item.accountId, item.stablecoinSymbol, item.chain, item.txHash, item.walletAddress].filter(Boolean).join(" ").toLowerCase();
    return matchesStatus && (!query || haystack.includes(query));
  });

  const filteredLedger = finance.walletLedger.filter((item) => {
    const haystack = [item.id, item.accountId, item.entryType, item.currency, item.source, item.referenceId, item.note].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    <Shell eyebrow="Finance" title="收入与结算">
      <StatsGrid
        items={[
          { label: "已结算收入", value: `$${formatNumber(finance.metrics.settledRevenueUsd)}`, hint: `${finance.metrics.confirmedOrders} 笔已确认订单`, tone: "accent" },
          { label: "待结算收入", value: `$${formatNumber(finance.metrics.pendingRevenueUsd)}`, hint: "确认中、复核中与待处理订单" },
          { label: "已发放 Token", value: formatNumber(finance.metrics.tokenGranted), hint: "所有正向 token 流水" },
          { label: "API 池续期", value: finance.metrics.apiPoolExpiring, hint: "即将到期的模型池账户", tone: "warn" },
        ]}
      />
      ...
      <div className="grid-2">
        <Panel title="最近结算订单" subtitle="查看支付、确认和结算进度。">
          <form className="toolbar-form" method="get">
            <div className="toolbar-form__field"><label className="toolbar-form__label" htmlFor="finance-q">搜索</label><input className="toolbar-form__input" defaultValue={filters.q ?? ""} id="finance-q" name="q" placeholder="orderId / txHash / 设备用户 / wallet ref" /></div>
            <div className="toolbar-form__field"><label className="toolbar-form__label" htmlFor="finance-status">状态</label><select className="toolbar-form__select" defaultValue={filters.status ?? ""} id="finance-status" name="status"><option value="">全部</option><option value="pending">待处理</option><option value="confirming">确认中</option><option value="reviewing">复核中</option><option value="confirmed">已确认</option><option value="failed">失败</option><option value="expired">已过期</option></select></div>
            <div className="toolbar-form__actions"><button className="toolbar-form__button" type="submit">应用</button><a className="toolbar-form__link" href="/finance">重置</a></div>
          </form>
          ...
          <SimpleTable
            ...
            columns={[
              ...
              { key: "status", header: "状态", cell: (row) => <StatusChip label={formatGenericStatus(row.status)} tone={row.status === "confirmed" ? "ok" : row.status === "confirming" || row.status === "reviewing" ? "warn" : "default"} /> },
              { key: "amount", header: "金额", cell: (row) => `$${row.amountUsd}` },
              { key: "tx", header: "Tx Hash", cell: (row) => row.txHash ?? "-" },
              { key: "order", header: "订单页", cell: (row) => <a className="result-link" href={`/orders?q=${encodeURIComponent(row.id)}`}>打开订单</a> },
            ]}
            emptyLabel="暂无匹配订单。"
          />
        </Panel>
        ...
        <div className="stack">
          <Panel title="财务概览" subtitle="封闭测试阶段的精简财务视角。">
