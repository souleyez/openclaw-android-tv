...
export default async function RiskPage() {
  ...
  return (
    <Shell eyebrow="Risk" title="续期与异常风险">
      <StatsGrid
        items={[
          { label: "即将到期权益", value: risk.metrics.expiringRights, hint: "14 天内到期的设备用户", tone: "warn" },
          { label: "失败订单", value: risk.metrics.failedOrders, hint: "失败或已过期的稳定币订单" },
          { label: "复核队列", value: risk.metrics.reviewOrders, hint: "等待人工处理的订单" },
          { label: "策略拦截", value: risk.metrics.blockedEvents, hint: "最近的权限或范围拦截", tone: "accent" },
        ]}
      />
      ...
      <div className="grid-2">
        <Panel title="模型链路风险" subtitle="快速看最近流量落在哪条链路上。">
          <div className="meta-list">
            <div className="meta-row"><span className="meta-row__label">客户端直连租约</span><strong>{risk.metrics.directLease}</strong></div>
            <div className="meta-row"><span className="meta-row__label">后端模型回退</span><strong>{risk.metrics.serverFallback}</strong></div>
            <div className="meta-row"><span className="meta-row__label">本地离线路由</span><strong>{risk.metrics.offlineLocal}</strong></div>
            <div className="meta-row"><span className="meta-row__label">API 池续期风险</span><strong>{risk.metrics.expiringApiPoolAccounts}</strong></div>
          </div>
        </Panel>
        ...
        <Panel title="API 池风险" subtitle="可能影响模型可用性和成本的上游账户。">
          <SimpleTable
            ...
            columns={[
              ...
              { key: "status", header: "状态", cell: (row) => <StatusChip label={formatGenericStatus(row.status)} tone="warn" /> },
        ...
        <Panel title="失败与过期订单" subtitle="可能需要客服介入或引导重试的支付事件。">
          <SimpleTable
            ...
            columns={[
              ...
              { key: "status", header: "状态", cell: (row) => <StatusChip label={formatGenericStatus(row.status)} tone="danger" /> },
