...
export default async function LogsPage({ searchParams }: LogsPageProps) {
  ...
  const blockedLogs = allLogs.filter((item) => item.modelProvider === "client_policy_guard").length;
  const directLeaseLogs = allLogs.filter((item) => item.transportMode === "client_direct_provider_lease").length;
  const tokenSpend = allLogs.reduce((sum, item) => sum + (item.tokenUsage ?? 0), 0);

  return (
    <Shell eyebrow="Observability" title="日志与审计">
      <StatsGrid
        items={[
      ...
      <Panel title="最近助手事件" subtitle="按用户、模式、链路和路由排查当前行为。">
        ...
        <SimpleTable
          ...
          columns={[
            ...
            { key: "transport", header: "链路", cell: (row) => formatTransportMode(row.transportMode) },
      ...
      <div className="grid-2">
        <Panel title="链路说明" subtitle="方便值班同学快速理解日志来源。">
          <div className="meta-list">
            <div className="meta-row"><span className="meta-row__label">客户端直连租约</span><strong>客户端拿临时凭证直连模型</strong></div>
            <div className="meta-row"><span className="meta-row__label">后端模型回退</span><strong>客户端直连失败后走服务端路由</strong></div>
            <div className="meta-row"><span className="meta-row__label">本地离线路由</span><strong>断网时仅使用本地控制链</strong></div>
          </div>
        </Panel>
        ...
        <Panel title="策略拦截摘要" subtitle="最近被范围或权限挡下来的请求。">
          <SimpleTable
            ...
            columns={[
              ...
              { key: "account", header: "设备用户", cell: (row) => row.accountId ?? "-" },
              { key: "route", header: "路由", cell: (row) => row.route ?? "-" },
              { key: "status", header: "状态", cell: () => <StatusChip label="paused" tone="warn" /> },
              { key: "input", header: "用户输入", cell: (row) => row.userText ?? "-" },
            ]}
            emptyLabel="暂无新的策略拦截。"
          />
