...
2026-05-16 补充：当前普通 APK P1 baseline 已完成一轮真机 acceptance smoke，最新厂商追料清单见 `docs/ops/2026-05-16-vendor-p1-follow-up.md`。X10 生产资料仍只能支持硬件口径核对，不能替代固件、BSP、投屏、OTA、诊断或投影硬件控制接口。
...
## 8. 2026-05-16 P1 追料优先级

| 优先级 | 资料 | 当前缺口影响 | 验收门槛 |
| --- | --- | --- | --- |
| P0 | 无线模块实物确认 | `W800CS/W8800DCS` 与 `8723DS` 口径冲突，影响 Wi-Fi/BT/投屏问题归因 | 明确实物模块、驱动、固件、SDIO/UART 配置、天线说明 |
| P0 | 当前工程固件与刷机 SOP | 无法做真实开机自启、系统投屏、系统服务、稳定性联调 | 固件能刷入、能回滚、有版本号和日志方式 |
| P0 | 系统身份和权限策略 | 普通 APK 无法承诺冻结系统包、系统投屏、OTA、诊断、硬件控制 | 平台签名、priv-app、权限白名单、SELinux 增量、预装升级策略明确 |
| P0 | 投屏系统能力 | APK 已可启动 DLNA，但苹果/小米镜像仍需系统 sink 或乐播 | Miracast/Wi-Fi Display/AirPlay sink 能力、入口、日志、sample 可运行 |
| P1 | OTA 和诊断 | home 后台无法形成设备升级和售后闭环 | OTA 状态 API、日志导出 SOP、crash/ANR/tombstone 获取方式 |
| P1 | 投影硬件 API | 对焦、梯形、亮度、输入源、电源、温度、风扇仍无法由 APP 控制 | AIDL/SDK/文档/sample/测试固件/错误码齐全 |

系统级能力只有在拿到接口文件或 SDK、方法文档、错误模型、demo APK 或 sample client、测试固件和目标板运行日志后，才从 `blocked` 改为 `development-ready`。
